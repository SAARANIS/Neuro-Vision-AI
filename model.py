...
from tensorflow.keras.losses import MeanSquaredError  # Updated import
...
def build_model(input_shape=(4, 160, 192, 128), output_channels=3, weight_L2=0.1, weight_KL=0.1, dice_e=1e-8):
    ...
    x = Lambda(sampling, output_shape=(128,), name='Dec_VAE_VDraw_Sampling')([z_mean, z_var])
    ...
    x = Add(name='Input_Dec_VAE_128')([x, x3])
    ...
    x = Add(name='Input_Dec_VAE_64')([x, x2])
    ...
    x = Add(name='Input_Dec_VAE_32')([x, x1])
    ...
    def loss_gt(e):
        # Implement your loss_gt function
        pass

    def loss_VAE(input_shape, z_mean, z_var):
        # Implement your loss_VAE function
        pass

    # Build and Compile the model
    # model = build_model(input_shape, output_channels)
    model = Model(inputs=inp, outputs=[out_GT, out_VAE])
    model.compile(optimizer=Adam(learning_rate=1e-4), loss=[loss_gt(dice_e), loss_VAE(input_shape, z_mean, z_var)],
        loss_weights=[1, weight_L2]
    )
